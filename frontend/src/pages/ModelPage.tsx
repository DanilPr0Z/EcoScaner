...
export function ModelPage() {
  ...
  useEffect(() => {
    ...
    const load = () => {
      api
        ...
        .then((next) => {
          setInfo(next);
          // Пока обучение идёт, подтягиваем цифры сами, чтобы не перезагружать
          // страницу. Обучение пишет прогресс каждые пять батчей — это примерно
          // раз в семь секунд, — поэтому обновляемся вдвое реже: полоса ползёт
          // заметно, а лишних запросов нет.
          if (next.inProgress) timer = window.setTimeout(load, 15_000);
        })
  ...
  // Итог последней досчитанной эпохи — опора для живой цифры в полосе прогресса.
  const finished = info.history.at(-1);
  const previousLoss =
    typeof finished?.trainLoss === "number" ? finished.trainLoss : null;
        ...
        <div className="progress">
          <div className="progress__head">
            ...
            <span className="progress__loss">
              ...
              {/*
                Это бегущее среднее внутри текущей эпохи: ultralytics обнуляет
                его на каждой эпохе и пересчитывает после каждой порции. Начало
                эпохи всегда тяжелее конца — модель ещё не поправилась, —
                поэтому число закономерно выше итогового.

                Рядом показываем итог прошлой эпохи: без него живая цифра висит
                без опоры, и каждый её подъём читается как ухудшение, хотя
                сравнивать надо законченные эпохи между собой.
              */}
              {previousLoss !== null && (
                <span className="progress__previous">
                  {" "}· прошлая эпоха: {previousLoss.toFixed(4)}
                </span>
              )}
            </span>
          </div>
